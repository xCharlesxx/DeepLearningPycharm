...
def download(key, secret, version, replays_dir, download_dir, extract=True,
             remove=False, filter_version='sort'):
    ...
    for i, meta_file_url in enumerate(sorted(meta_file_urls), 1):
        ...
        if True:
            ...
            if os.path.getsize(file_path) <= 22:  # Size of an empty zip file.
                ...
                ZipFile.extractall(ZipFile(file_path), replays_dir, pwd=b'iagreetotheeula')
            if False:
                os.remove(file_path)
...
def main():
    ...
    parser.add_argument('--version', default='4.9.1', help='Download all replays from this StarCraft 2 game version, eg: "4.8.3".')
    parser.add_argument('--replays_dir', default='C:\\replays9', help='Where to save the replays.')
    parser.add_argument('--download_dir', default='C:\\download9', help='Where to save the zip files.')
